...
kernel_about = f"Builders Inteligence is running threw the Cyclone Kernel via Python\nKernel Version: {kernel_version} Code Platform: {kernel_cmds}\n{kernel_platform}"
...
def var_get(varname):
    try:
        ...
        print(f"[BuildersInteligence] Code 11")
        product = "Uh Oh"
    return product
...
def runcmd(cmd):
    if cmd["type"]=="#":
        ...
        if cmd["args"][0].lower()=="about":
            print(kernel_about)
    else: print(f"[BuildersInteligence] Command \u001b[4m"+cmd["type"]+"\u001b[0m is not Valid")
